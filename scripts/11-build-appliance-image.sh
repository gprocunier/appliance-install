...
#### These steps build the OpenShift appliance disk image on foundry

# Run this script from the operator workstation, in the repository root.
# The build runs inside the appliance builder container on foundry.
...
#### These steps validate foundry build inputs

# The generated appliance config contains pull-secret content and stays on foundry.
if [[ ! -f "\${ASSETS_DIR}/appliance-config.yaml" ]]; then
    echo "Missing \${ASSETS_DIR}/appliance-config.yaml. Run script 10 first." >&2
    exit 1
fi

echo "Foundry appliance build workspace:"
df -h "\${ASSETS_DIR}" /
echo
...
# The build mirrors OCP and operator content and can be quiet for long periods.
echo "Starting appliance image build at \$(date -Is)."
echo "This phase mirrors OpenShift and operator content into \${ASSETS_DIR}."
echo "Builder log on foundry: \${ASSETS_DIR}/.openshift_appliance.log"
echo "oc-mirror logs on foundry: \${ASSETS_DIR}/temp/oc-mirror/working-dir/logs/"
podman run --rm \
    --pull newer \
    --privileged \
    --net=host \
...
echo "OpenShift appliance image build is complete."
REMOTE_SCRIPT
