...
| `scripts/02-install-host-packages.sh` | Operator workstation | Installs packages on the remote virtualization host. |
| `scripts/03-enable-host-services.sh` | Operator workstation | Enables services on the remote virtualization host. |
...
| `scripts/07-configure-foundry-services.sh` | Operator workstation | Configures foundry through the virtualization host jump path. |
| `scripts/08-verify-foundry-services.sh` | Operator workstation | Verifies foundry DNS, NTP, and staging services. |
...
VM work is still launched from the operator workstation. Scripts that configure
services inside foundry use SSH through the virtualization host jump path.
